...
export function listZipEntries(zip) {
    ...
    if (eocd < 0)
        throw new Error("INVALID_ZIP_EOCD");
    ...
    while (p < end) {
        if (u32(b, p) !== 0x02014b50)
            throw new Error("INVALID_ZIP_CDH");
...
export function extractZipEntry(zip, entry) {
    const b = zip;
    const p = entry.offset;
    if (u32(b, p) !== 0x04034b50)
        throw new Error("INVALID_ZIP_LH");
    ...
    if (entry.method === 0)
        return comp; // stored
    if (entry.method === 8)
        return inflateRawSync(comp); // deflate
