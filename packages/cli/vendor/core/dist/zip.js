...
function normalizePath(p) {
    if (!p || p.includes("\\") || p.startsWith("/"))
        throw new Error("zipDeterministic: invalid path");
    if (p.includes(".."))
        throw new Error("zipDeterministic: path traversal");
    return p;
}
export function zipDeterministic(entries) {
    ...
    for (const e of sorted) {
        ...
        const compressed = method === 8 && data.length
